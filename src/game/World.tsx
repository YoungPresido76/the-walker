...
function useAssetTexture(path: string, repeat: number) {
  return useMemo(() => {
    const texture = new THREE.TextureLoader().load(`/the-walker/${path}`);
...
function ColoredInstances({
  ...
  return (
    ...
    >
      ...
      <meshStandardMaterial map={map ?? undefined} roughness={roughness} metalness={0.03} flatShading={flatShading} />
    </instancedMesh>
  );
}
...
function Weather() {
  ...
  return <points ref={ref}><bufferGeometry><bufferAttribute attach="attributes-position" args={[positions, 3]} count={positions.length / 3} array={positions} itemSize={3} /></bufferGeometry><pointsMaterial color="#b2c5bb" size={0.055} transparent opacity={0.3} depthWrite={false} /></points>;
}
  ...
  return (
    <>
      <mesh position={[0, 4.2, 0]} rotation={[Math.PI / 2, 0, 0]}>
        ...
        <meshStandardMaterial map={stone} color="#202832" roughness={1} side={THREE.DoubleSide} />
      </mesh>
      <mesh position={[x, 1.55, z - 1.88]} rotation={[0, Math.PI, 0]}>
        <planeGeometry args={[1.35, 1.35]} />
        <meshBasicMaterial map={rune} color="#f0ad68" transparent opacity={0.72} side={THREE.DoubleSide} depthWrite={false} />
      </mesh>
      <group ref={figure} position={[x, 0, z - 1.4]} visible={false}>
        <mesh position={[0, 1.2, 0]} castShadow>
          <coneGeometry args={[0.7, 2.7, 7]} />
          <meshStandardMaterial color="#040508" roughness={1} transparent opacity={0.96} />
        </mesh>
        <mesh position={[0, 2.72, 0]}>
          <sphereGeometry args={[0.35, 12, 8]} />
          <meshStandardMaterial color="#050507" roughness={1} />
        </mesh>
        <mesh position={[-0.12, 2.76, -0.31]}>
...
function Gate({ maze, runtime }: { maze: Maze; runtime: Runtime }) {
  ...
  return (
    <group ref={group} position={[x, 0, z]} visible={false}>
      <mesh position={[-span, h / 2, 0]} castShadow><boxGeometry args={[post, h, post]} /><meshStandardMaterial color={WORLD.wood} roughness={0.92} /></mesh>
      <mesh position={[span, h / 2, 0]} castShadow><boxGeometry args={[post, h, post]} /><meshStandardMaterial color={WORLD.wood} roughness={0.92} /></mesh>
      <mesh position={[0, h + 0.12, 0]} castShadow><boxGeometry args={[span * 2 + 0.5, 0.28, 0.32]} /><meshStandardMaterial color="#2c1e14" roughness={0.9} /></mesh>
      <mesh position={[0, h - 0.12, 0.02]}><boxGeometry args={[span * 2 - 0.1, 0.08, 0.08]} /><meshStandardMaterial color={WORLD.exitGlow} emissive={WORLD.exitGlow} emissiveIntensity={0.7} /></mesh>
      <mesh position={[0, 2.55, 0.18]}><sphereGeometry args={[0.13, 8, 8]} /><meshStandardMaterial color={WORLD.exitGlow} emissive={WORLD.exitGlow} emissiveIntensity={2.2} /></mesh>
      <pointLight position={[0, 2.5, 0.3]} color={WORLD.sun} intensity={4.2} distance={11} decay={2} />
      <mesh position={[0, 0.01, -0.9]} rotation={[-Math.PI / 2, 0, 0]}><circleGeometry args={[1.35, 12]} /><meshBasicMaterial color={WORLD.exitGlow} transparent opacity={0.14} /></mesh>
    </group>
  );
}
...
function HintArrow({ runtime }: { runtime: Runtime }) {
  ...
  return <group ref={ref} visible={false}><mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0, 0.28]}><coneGeometry args={[0.16, 0.42, 5]} /><meshBasicMaterial ref={mat} color={WORLD.orb} transparent opacity={0.85} depthWrite={false} /></mesh></group>;
}

export function World({ maze, runtime }: { maze: Maze; runtime: Runtime }) {
  const grass = useGrassTexture();
  const stone = useAssetTexture("assets/underground-wall.png", 22);
  const rune = useAssetTexture("assets/rune-mark.png", 1);
  ...
  const underground = runtime.mode === "underground";
  return (
    <>
      <color attach="background" args={[underground ? "#05080d" : WORLD.sky]} />
      <fog attach="fog" args={[underground ? "#091019" : WORLD.fog, underground ? 2.2 : WORLD.fogNear, underground ? 25 : WORLD.fogFar]} />
      <hemisphereLight args={underground ? ["#1f3045", "#05070b", 0.24] : ["#5a7481", "#14291b", 0.45]} />
      <ambientLight intensity={underground ? 0.08 : 0.13} color={underground ? "#34465a" : "#9fb7c4"} />
      <directionalLight position={[18, 26, 10]} intensity={underground ? 0.02 : 0.62} color="#c5d5e5" castShadow={!cheap} shadow-mapSize={[1024, 1024]} shadow-camera-near={2} shadow-camera-far={140} shadow-camera-left={-34} shadow-camera-right={34} shadow-camera-top={34} shadow-camera-bottom={-34} shadow-bias={-0.0005} />
      ...
      <ColoredInstances items={maze.walls} castShadow={!cheap} receiveShadow roughness={0.9} flatShading map={underground ? stone : null} />
      <ColoredInstances items={maze.caps} castShadow={!cheap} roughness={0.84} flatShading map={underground ? stone : null} />
      <ColoredInstances items={maze.clumps} shape="ico" roughness={0.84} flatShading />

      {!underground ? <Trees maze={maze} /> : null}
      {!underground ? <NatureLandmarks maze={maze} /> : null}
      {!underground ? <Weather /> : null}
      <Gate maze={maze} runtime={runtime} />
      <Orbs maze={maze} runtime={runtime} />
      <HintArrow runtime={runtime} />
      {underground ? <UndergroundDetails maze={maze} runtime={runtime} stone={stone} rune={rune} /> : null}
      <Flashlight runtime={runtime} />

      {!underground ? <mesh position={[cx, WALL_HEIGHT + 6, cz]} rotation={[Math.PI, 0, 0]}><sphereGeometry args={[220, 20, 10, 0, Math.PI * 2, 0, Math.PI / 2]} /><meshBasicMaterial color={WORLD.sky} side={THREE.BackSide} fog={false} /></mesh> : null}
    </>
  );
}
